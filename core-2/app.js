 ...
 *   CRC     : 1 byte
 */
function buildPacket(cmd, data = [], direction = 'app->device') {
...
function parsePacket(raw) {
  try {
    ...
    return {
      ...
      direction,
      cmd,
      cmdName: COMMANDS[cmd] ?? `UNKNOWN_0x${cmd.toString(16).toUpperCase().padStart(2, '0')}`,
      data,
      crc: receivedCRC,
      declaredLength,
      ...
      <View style={styles.columns}>
        ...
        <View style={[styles.panel, styles.devicePanel]}>
          ...
          <View style={styles.buttonRow}>
            <PanelButton
              label="−10%"
              onPress={() => deviceChangeBattery(-10)}
          ...
          <PanelButton
            ...
            onPress={() => {
              deviceToggleCharging();
              setTimeout(deviceSendChargingStatus, 50);
            }}
            disabled={!isConnected}
            color="#8e44ad"
            fullWidth
          ...
          <PanelButton
            ...
            onPress={() => {
              setDeviceState(prev => ({ ...prev, worn: !prev.worn }));
              setTimeout(() => deviceSendActionSync({ worn: !deviceState.worn }), 50);
            }}
            disabled={!isConnected}
            color="#c0392b"
            fullWidth
...
function PanelButton({ label, onPress, disabled, color, fullWidth }) {
  return (
    <TouchableOpacity
      style={[
        styles.panelBtn,
